...
ln -s /path/to/project/.claude .claude
```

**Verification**:
```bash
ls -la .claude
# Should show: .claude -> /path/to/project/.claude
...
ln -s /path/to/project/.claude/skills .claude/skills
ln -s /path/to/project/.claude/commands .claude/commands

# Keep project-specific config
cp /path/to/project/.claude/mcp.json.example .claude/mcp.json
```
...
**Core Projects**:
- `/path/to/project` - **Source of truth** (main repository)
- `/path/to/website-project` - Website project
...
```bash
readlink .claude
# Should output: /path/to/project/.claude
```
...
vim /path/to/project/.claude/skills/email-campaign-management.md
...
CORE_CLAUDE="/path/to/project/.claude"

#!/usr/bin/env python3
"""
Validate AINative Adapter Quality (CPU/MPS Version)

Apple Silicon / CPU-compatible version of the adapter validation script.
This version bypasses bitsandbytes quantization and uses FP16 precision instead.
...
def check_adapter_files(adapter_path: Path) -> Tuple[bool, List[str]]:
    """
    Check if adapter directory contains required files.
...
def load_adapter_cpu(adapter_path: Path, base_model: str = "unsloth/Llama-3.2-1B-Instruct") -> Tuple:
    """Load the base model and adapter for CPU/MPS (no quantization)."""
    ...
    print("⚠️  Running without 4-bit quantization - using FP16 precision")
    print("💡 This will use ~4-6GB RAM vs ~1-2GB with quantization\n")
...
def main():
    ...
    print("🚀 AINative Adapter Validation (CPU/MPS Version)")
    ...
    # Check adapter files
    is_valid, missing_files = check_adapter_files(adapter_path)
    if not is_valid:
    ...
    print(f"✅ Adapter files validated")
    print(f"📊 Tests: {len(VALIDATION_TESTS)}\n")

    # Load adapter
    try:
        model, tokenizer = load_adapter_cpu(adapter_path)
    ...
    # Save results
    output_file = Path(__file__).parent.parent / "outputs" / "ainative_adapter_validation_cpu.json"
